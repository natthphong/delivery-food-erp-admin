...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    return res.status(200).json({ code: "OK", message: "success", body: { id, status: "CONFIRMED" } });
  } catch (error) {
    console.error("orders/[id]/confirm error", error);
    return res.status(500).json({ code: "SERVER_ERROR", message: "Unable to confirm order" });
  }
}
