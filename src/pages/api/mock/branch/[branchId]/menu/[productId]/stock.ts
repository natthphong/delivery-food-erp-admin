...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { stock_qty } = (req.body ?? {}) as { stock_qty?: number | null | string };
  ...
  try {
    ...
    const parsedValue = stock_qty === null || stock_qty === "" ? null : Number(stock_qty);
    item.stock_qty = parsedValue;
    await setKV(key, data);
    return res.status(200).json({ code: "OK", message: "success", body: { productId, stock_qty: item.stock_qty } });
  } catch (error) {
    console.error("branch/menu/stock error", error);
    return res.status(500).json({ code: "SERVER_ERROR", message: "Unable to update stock" });
  }
}
