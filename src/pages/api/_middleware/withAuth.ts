...
export type JsonResponse = { code: string; message: string; body: any } | { code: string; message: string };
...
if (payload?.sub) {
return { uid: payload.uid ?? payload.sub, userId: typeof payload.userId === “number” ? payload.userId : null, tokenType: “access”, sub: payload.sub };
}
} catch {}
}
...
if (!uid) return null;
return { uid: String(uid), userId: null, tokenType: “idToken”, firebaseClaims: claims as Record<string, any> };
} catch {
return null;
}
}
...
if (!auth) return res.status(401).json({ code: “UNAUTHORIZED”, message: “Missing or invalid token” });
